...
class _ArMeasurementScreenState extends State<ArMeasurementScreen> {
  ...
  void onARKitViewCreated(ARKitController arkitController) {
    ...
    this.arkitController?.onARTap = (List<ARKitTestResult> ar) {
      ...
      if (planeTap.type == ARKitHitTestResultType.existingPlaneUsingExtent) {
  ...
  void _handleUpdateAnchor(ARKitAnchor anchor) {
    if (anchor.identifier != anchorId || node == null || plane == null) {
      return;
    }
  ...
  void _onPlaneTapHandler(Matrix4 transform) {
    ...
    if (lastPosition != null) {
      final line = ARKitLine(
        ...
        materials: [
          ARKitMaterial(
              diffuse: const ARKitMaterialProperty(
                  color: Color.fromRGBO(255, 153, 83, 1))),
        ],
      );
